...
second_design_list = {dc_dec, dcudp_cntl, wrbuf_cntl, miss_cntl, \
		      dcctl, dcu_dpath, aligner, swapper, buffer, dcu}
...
report_area > db/dcu_area.rpt
report_timing -path end -delay max -max_paths PATHS_END -nosplit > db/dcu_maxend.rpt
report_timing -path full -delay max -max_paths PATHS_FULL > db/dcu_maxtim.rpt
report_timing -path end -nets -delay max -max_paths PATHS_END -nosplit > db/dcu_violators.rpt
report_constraints -all_violators -verbose >> db/dcu_violators.rpt
 
/* write out the results */
write -f db -h -o db/dcu.db {dcu}
write -f verilog -h -o db/dcu.vSyn {dcu}
current_design dcu
ungroup -flatten -all
write -f verilog -o db/dcu_gate.vSyn {dcu}
