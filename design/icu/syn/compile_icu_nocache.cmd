...
second_design_list = {ibuf_ctl, encode_shift_module, ibuf_ctl_slice, \
                      ...
                      ic_aligner, ic_len_decoder, icu_nocache}
...
report_area > db/icu_nocache_area.rpt
report_timing -path end -delay max -max_paths PATHS_END -nosplit > db/icu_nocache_maxend.rpt
report_timing -path full -delay max -max_paths PATHS_FULL > db/icu_nocache_maxtim.rpt
report_timing -path end -nets -delay max -max_paths PATHS_END -nosplit > db/icu_nocache_violators.rpt
report_constraints -all_violators -verbose >> db/icu_nocache_violators.rpt
 
/* write out the results */
write -f db -h -o db/icu_nocache.db {icu_nocache}
write -f verilog -o db/icu_nocache.vSyn {icu_nocache}
current_design icu_nocache
ungroup -flatten -all
write -f verilog -o db/icu_nocache_gate.vSyn {icu_nocache}
