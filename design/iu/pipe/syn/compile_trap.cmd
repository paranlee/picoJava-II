...
report_area > db/trap_area.rpt
report_timing -path end -delay max -max_paths PATHS_END -nosplit > db/trap_maxend.rpt
report_timing -path full -delay max -max_paths PATHS_FULL > db/trap_maxtim.rpt
report_timing -path end -nets -delay max -max_paths PATHS_END -nosplit > db/trap_violators.rpt
report_constraints -all_violators -verbose >> db/trap_violators.rpt

/* write out the results */
write -f db -h -o db/trap.db {trap} 
write -f verilog -h -o db/trap.vSyn {trap} 
