...
second_design_list = {pipe_dpath, pipe_cntl, hold_logic, pipe}
...
report_area > db/pipe_area.rpt
report_timing -path end -delay max -max_paths PATHS_END -nosplit > db/pipe_maxend.rpt
report_timing -path full -delay max -max_paths PATHS_FULL > db/pipe_maxtim.rpt
report_timing -path end -nets -delay max -max_paths PATHS_END -nosplit > db/pipe_violators.rpt
report_constraints -all_violators -verbose >> db/pipe_violators.rpt

/* write out the results */
write -f db -h -o db/pipe.db {pipe} 
write -f verilog -h -o db/pipe.vSyn {pipe} 
